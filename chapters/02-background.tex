...
% PZIE: Maybe "This task is mainly handled by hypervisors such as VMWare or Hyper-V (so called level one and two hypervisors)." Otherwise you have to explain what are lev1 and lev2 hyp
...
% PZIE: suggestion: "availability than the primary storage source, reducing the time and resources required to retrieve this data."
% Otherwise you suggest you reduce time to retrieve from primary source - not true
...
allowing for faster access on subsequent requests. Another place where caches are used are operating systems, where caches are frequently used to store contents and metadata of % XXX: rewrite, this feels like a 12yo wrote it (whole paragraph)
...
\begin{center}
	...
	\label{fig:memhier}
\end{center}
% PZIE: Is it yours? Otherwise - link to sourece
...
\begin{itemize} % TODO: NASTY: citations for EVERYTHING here, I'm putting a ballpark figures here! marking as nasty as Im 100% sure that backing this up will be a _major_ pain in the ass
	...
	\item \textbf{Long term storage:} refers to external storage solutions and archival media, such as tape drives. Access times can vary widely, in case of tape drivers it is in range of several
		seconds to minutes.
\end{itemize}
% PZIE In general the above is quite bold. Especially time values. But if you can back them up, then it's fine
...
% PZIE "they require careful management" sounds a bit better maybe?
...
\begin{itemize}
	...
	% PZIE: Dropped "the" here and there. With uncountable nouns it's tricky. I also would recommend using "cache" as uncountable noun, I believe it's fairly common
...
\]
% PZIE: Frankly, you don't use `m`' :P

\noindent Let's consider a practical example with typical values for a modern CPU:
% PZIE 95% seems crazy high. Is this for a particular scenario? Running something specific?
...
\begin{itemize}
	\item \textbf{Increasing cache size:} Larger caches can store more data, reducing the likelihood of cache misses. However, this approach does not scale well,
	    ...
	    % PZIE: space and power - you already mentioned space
...
\end{itemize}
% PZIE: you never introduce what a line is and what a block is. 
...
devices, such as embedded microcontrollers, often opt for simple implementations, such as \textit{random eviction} or \textit{FIFO}, as these require % TODO: add \ref to these methods
...
% PZIE: I'd drop the "increasing power consumption". It appeared a few times alreayd
...
% PZIE: I have a feeling that there should be sth like "The goal of cache coherency SOMETHING is to maintain..", like there should be a noun. I'm not sure tbh. I have a FEELING that coherency is a state, not a process
...
\begin{itemize}
	\item \textbf{I/O to memory DMA:} In this mode, the CPU initiates a data block % TODO: back this up
...
The contents of the first level of cache are usually not shared between cores. Despite L1 being private to each core, coherence must be maintained across all cache levels. % TODO: footnote explaining cache sync vs cache cocherence
